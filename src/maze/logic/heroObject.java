...
public class heroObject extends ObjectIOMove {
	...
	public void move(char direction) {
		...
		if (MazeGame.maze.mazeMap[MazeGame.maze.hero.getY() + y][MazeGame.maze.hero
				.getX() + x] != 'X'
				&& !(MazeGame.maze.hero.getX() + x == MazeGame.maze.exit.getX()
						&& MazeGame.maze.hero.getY() + y == MazeGame.maze.exit
								.getY() && MazeGame.maze.hero.getState() == 'H')) {
			MazeGame.maze.mazeMap[MazeGame.maze.hero.getY()][MazeGame.maze.hero
					.getX()] = ' ';
			MazeGame.maze.mazeMap[MazeGame.maze.hero.getY() + y][MazeGame.maze.hero
					.getX() + x] = MazeGame.maze.hero.getState();
			MazeGame.updateObject(MazeGame.maze.hero, MazeGame.maze.hero.getX()
					+ x, MazeGame.maze.hero.getY() + y,
		...
		if (GameObject.samePosition(MazeGame.maze.hero, MazeGame.maze.sword)) {
			...
			MazeGame.maze.mazeMap[MazeGame.maze.hero.getY()][MazeGame.maze.hero
					.getX()] = MazeGame.maze.hero.getState();
		}
		...
		for (int i = 0; i < MazeGame.maze.dragons.size(); i++)
			if (GameObject.adjacentPosition(MazeGame.maze.hero,
				...
				MazeGame.maze.mazeMap[MazeGame.maze.dragons.get(i).getY()][MazeGame.maze.dragons.get(i)
						.getX()] = ' ';
				// bug fix 1
